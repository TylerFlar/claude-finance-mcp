...
async def scrape_balances(page: Page) -> list[dict]:
    await page.goto(BOFA_OVERVIEW_URL, wait_until="domcontentloaded")
    await wait_for_navigation(page)
    await check_logged_in(page, "bofa")
    ...
    bank_count = await _safe_count(bank_rows)
    ...
    credit_count = await _safe_count(credit_rows)
    ...
    for i in range(credit_count):
        ...
        card_name = (
            name_match.group(1).strip() if name_match else "BofA Credit Card"
        )
    ...
    if not balances:
        ...
        if checking_match:
            balances.append({
                ...
                "type": "checking",
                "current_balance": (
                    parse_dollar_amount(checking_match.group(1)) or 0
                ),
                "available_balance": parse_dollar_amount(
                    checking_match.group(1)
                ),
                "currency": "USD",
            })
        ...
        if credit_match:
            balances.append({
                ...
                "type": "credit",
                "current_balance": (
                    parse_dollar_amount(credit_match.group(1)) or 0
                ),
                "available_balance": None,
                "currency": "USD",
            })
...
async def scrape_transactions(page: Page, days_back: int) -> list[dict]:
    ...
    checking_link = page.locator(
        "a:has-text('Checking'), "
        "a[class*='account-name']:has-text('Checking')"
    ).first
    try:
        if await checking_link.is_visible(timeout=5000):
            await checking_link.click()
            await wait_for_navigation(page)
            await _scrape_transaction_rows(
                page, transactions, "BofA Checking", cutoff
            )
            await page.go_back()
            await wait_for_navigation(page)
    except Exception:
    ...
    try:
        if await credit_link.is_visible(timeout=5000):
            await credit_link.click()
            await wait_for_navigation(page)
            await _scrape_transaction_rows(
                page, transactions, "BofA Credit Card", cutoff
            )
    except Exception:
        pass
...
async def _scrape_transaction_rows(
...
async def scrape_credit_due(page: Page) -> dict | None:
    ...
    try:
        if not await credit_link.is_visible(timeout=5000):
            return None
    except Exception:
        return None
...
async def transfer(
    page: Page, from_account: str, to_account: str, amount: float
) -> dict:
    await page.goto(BOFA_TRANSFER_URL, wait_until="domcontentloaded")
    await wait_for_navigation(page)
    await check_logged_in(page, "bofa")
...
async def pay_credit_card(
    page: Page, amount: str | float, from_account: str
) -> dict:
    await page.goto(BOFA_BILLPAY_URL, wait_until="domcontentloaded")
    await wait_for_navigation(page)
    await check_logged_in(page, "bofa")
    ...
    if await from_select.is_visible():
        pay_options = await from_select.locator("option").all_text_contents()
        pay_match = next(
            (o for o in pay_options if re.search(from_account, o, re.I)), None
        )
        if pay_match:
            await from_select.select_option(label=pay_match)
